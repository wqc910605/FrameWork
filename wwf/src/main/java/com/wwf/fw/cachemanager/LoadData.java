...
import com.wwf.fw.utils.GsonUtil;
...
public class LoadData {
    ...
    //获取json对象的方法
    public <T> T getBeanData(String url, Class<T> clazz, HashMap<String,String> params, String cookValue) {

        //1. 去网络获取数据
        String content = HttpManager.getInstance().dataGet(url, params,cookValue);
...
//        return null;
    ...
    //获取json对象的方法
    public <T> List<T> getListData(String url, Type type, HashMap<String,String> params, String cookieValue) {

        //1. 去网络获取数据
        String content = HttpManager.getInstance().dataGet(url, params,cookieValue);
        //2. 判断当前的数据是否为空
        if (TextUtils.isEmpty(content)) {
            //如果是空
            //去缓存类中去取数据
